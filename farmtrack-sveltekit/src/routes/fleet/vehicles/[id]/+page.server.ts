...
export const load: PageServerLoad = async ({ params }) => {
	...
	// Fetch fuel entries for this vehicle (last 100 entries)
	const { data: fuelEntries, error: entriesError } = await supabase
		.from('fuel_entries')
		.select(`
			*,
	...
	// Fetch monthly consumption statistics
	...
	// Fetch consumption by activity (handle missing function gracefully)
	let activityStats = [];
	try {
		const result = await supabase
			.rpc('get_vehicle_activity_stats', { vehicle_id_param: id });
		activityStats = result.data || [];
	} catch (error) {
		console.warn('Activity stats function not available:', error);
	}
	...
	const avgConsumption = validConsumptionEntries.length > 0
		? validConsumptionEntries.reduce((sum, e) => sum + e.fuel_consumption_l_per_100km!, 0) / validConsumptionEntries.length
		: null;
